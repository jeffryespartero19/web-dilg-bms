...
//dd($cityX);
        ...
        $countX = count($cityIDs);
        $data = [];
//dd($cityIDs,$provIDs,$regionIDs);
        for ($i = 0; $i < $countX; $i++) {
            $provPSGC = DB::table('maintenance_province')->where('Province_ID', $provIDs[$i])->pluck('PSGC_code');
            $regionPSGC = DB::table('maintenance_region')->where('Region_ID', $regionIDs[$i])->pluck('PSGC_code');
            ...
            $dataX = DB::table('maintenance_barangay as a')
                ...
                ->leftjoin('maintenance_region as d', 'd.Region_ID', '=', DB::raw($regionIDs[$i]))
                ->select('a.Barangay_Name', 'b.City_Municipality_Name', 'c.Province_Name', 'd.Region_Name', 'a.Barangay_ID')
                ->where('a.Region_ID', $regionPSGC)
                ->where('a.Province_ID', $provPSGC)
                ->where('a.City_Municipality_ID', $cityPSGC[$i])
                ->get();
