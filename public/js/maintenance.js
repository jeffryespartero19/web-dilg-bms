...
// Frequency
//post buttons
$(document).on('click', '.postThis_Frequency', function(e) {
    $('#newBRGY_Frequency').submit();
});

$(document).on('click', ('.edit_frequency'), function(e) {

    var disID = $(this).val();
    $.ajax({
        url: "/get_frequency_maint",
        type: 'GET',
        data: { id: disID },
        fail: function() {
            alert('request failed');
        ...
        success: function(data) {
            $('#this_frequency_idX').val(data['theEntry'][0]['Frequency_ID']);
            $('#this_frequencyX').val(data['theEntry'][0]['Frequency']);

            $('#this_frequency_active').empty();
            $('#this_frequency_active').val(data['theEntry'][0]['Active']);
            if (data['theEntry'][0]['Active'] == 1) {
                $('#this_frequency_active').append('Yes');
            } else {
                $('#this_frequency_active').append('No');
            }
...
$(document).on('click', '.updateThis_Frequency', function(e) {
...
$(document).on('click', ('.edit_categories'), function(e) {
    ...
    $.ajax({
        ...
        success: function(data) {
            $('#this_categories_idX').val(data['theEntry'][0]['Categories_ID']);
            $('#this_categoriesX').val(data['theEntry'][0]['Categories']);

            $('#this_categories_active').empty();
            $('#this_categories_active').val(data['theEntry'][0]['Active']);
            if (data['theEntry'][0]['Active'] == 1) {
                $('#this_categories_active').append('Yes');
            } else {
                $('#this_categories_active').append('No');
            }
